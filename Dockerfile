...
RUN python -c "import os; from transformers import AutoModel, AutoTokenizer; t = os.environ.get('HF_TOKEN');\nif t:\n    AutoTokenizer.from_pretrained('ai4bharat/indic-bert', token=t)\n    AutoModel.from_pretrained('ai4bharat/indic-bert', token=t)\nelse:\n    print('WARNING: HF_TOKEN build argument missing. Skipping pre-download. Model will download at runtime.')"
